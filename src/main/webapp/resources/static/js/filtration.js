...
function fillDataType(arr) {
    ...
    for (var j = 0; j < arr.length; j++) {
        countPoints = 0;
        if (typeSel == "Не выбрано" || typeSel == arr[j].category) {
            countPoints++;
        }
        if (waySel == "Не выбрано" || waySel == arr[j].way) {
            countPoints++;
        }
        if (categorySel == "Не выбрано" || categorySel == arr[j].name) {
            countPoints++;
        }
        if (countPoints == 3) {
            filterData.push(arr[j]);
        }
    }
